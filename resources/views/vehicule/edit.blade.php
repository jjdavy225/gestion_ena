    ...
    <div class="container">
        <form action="{{ route('vehicule.update', $vehicule->id) }}" method="post">
            ...
            <div class="container mt-3">
                <h3 class="text-center">Modifier les infos du véhicule</h3>
            </div>
                ...
                <div class="col-lg-6">
                    ...
                    <div class="form-group mb-3">
                        ...
                        <input required class="form-control" type="text" name="carte_grise" id="carte_grise"
                            value="{{ old('carte_grise', $vehicule->carte_grise) }}">
                        @error('carte_grise')
                            <div class="alert alert-danger">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="form-group mb-3">
                        ...
                        <input required class="form-control" type="text" name="chassis" id="chassis"
                            value="{{ old('chassis', $vehicule->num_chassis) }}">
                        @error('chassis')
                            <div class="alert alert-danger">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="form-group mb-3">
                        ...
                        <input required class="form-control" type="date" name="date_circulation" id="date_circulation"
                            value="{{ old('date_circulation', $vehicule->date_mise_en_circulation) }}">
                    ...
                    <div class="toChange form-group mb-3">
                        ...
                        <select required class="selectChanger form-select" name="type_acquisition">
                            ...
                            @foreach (App\Models\Vehicule::distinct()->pluck('type_acquisition') as $type)
                                <option @if ($type === old('type_acquisition', $vehicule->type_acquisition)) {{ 'selected' }} @endif>{{ $type }}
                                </option>
                            @endforeach
                ...
                <div class="col-lg-6">
                    <div class="form-group mb-3">
                        ...
                        <input required class="form-control" type="date" name="date_acquisition" id="date_acquisition"
                            value="{{ old('date_acquisition', $vehicule->date_acquisition) }}">
                        @error('date_acquisition')
                            <div class="alert alert-danger">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="form-group mb-3">
                        <label class="form-label" for="kilometrage">Kilométrage au compteur</label>
                        <input class="form-control" type="number" placeholder="Défaut(0km)" name="kilometrage"
                            id="kilometrage" value="{{ old('kilometrage', $vehicule->kilometrage) }}">
                        @error('kilometrage')
                            <div class="alert alert-danger">{{ $message }}</div>
                        @enderror
                    </div>
                        ...
                        <select required class="form-select" name="fournisseur">
                            ...
                            @foreach ($fournisseurs as $fournisseur)
                                <option value="{{ $fournisseur->id }}"
                                    @if ($fournisseur->id === old('fournisseur', $fournisseur->id)) {{ 'selected' }} @endif>
                                    {{ $fournisseur->sigle }}</option>
                            @endforeach
                        </select>
                        @error('fournisseur')
                            <div class="alert alert-danger">{{ $message }}</div>
                        ...
                        <select required class="form-select" name="modele">
                            ...
                            @foreach ($modeles as $modele)
                                <option value="{{ $modele->id }}"
                                    @if ($modele->id === old('modele', $modele->id)) {{ 'selected' }} @endif>
                                    {{ $modele->designation }}</option>
                            @endforeach
                        </select>
                        @error('modele')
                            <div class="alert alert-danger">{{ $message }}</div>
                        ...
                        <select required class="selectChanger form-select" name="marque">
                            ...
                            @foreach ($marques as $marque)
                                <option value="{{ $marque->id }}"
                                    @if ($marque->id === old('marque', $marque->id)) {{ 'selected' }} @endif>
