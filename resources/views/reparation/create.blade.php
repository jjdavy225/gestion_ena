    ...
    <div class="container">
        <form action="{{ route('reparation.store') }}" method="post">
            ...
            <div class="container mt-3">
                <h3 class="text-center">Renseignez les infos de la réparation</h3>
            </div>
                ...
                <div class="col-lg-6">
                    <div class="form-group mb-3">
                        ...
                        <select required class="form-select @error('panne') is-invalid @enderror" name="panne">
                            ...
                            @foreach ($pannes as $panne)
                                <option value="{{ $panne->id }}">{{ $panne->code }} | N° Immat : {{ $panne->vehicule->immatriculation }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group mb-3">
                        ...
                        <input required class="form-control @error('montant') is-invalid @enderror" type="number"
                            name="montant" id="montant" value="{{ old('montant') }}" min="0">
                        <div class="invalid-feedback">
                            @error('montant')
                                {{ $message }}
                            @enderror
